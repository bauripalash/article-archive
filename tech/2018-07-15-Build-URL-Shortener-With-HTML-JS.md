...
You might be wondering how these things work? To understand how these things work, we need to take a closer look at a URL shortener so we’ll be building a simple URL shortener. With That Task, we’ll be learning some new things as well as understand how URL Shortener works.
...
As I showed in the above code, I've created a simple HTML file. Inside body tags, there're only three elements an `input`, a `button` and a `script`.
...
In the end we have a `script` called `main.js` where all our main javascript codes will be in, above mentioned `shorturl()` function will be also there.
...
So, now we'll create a function called `getrandom()`
...
At first, we check if the hash location is empty if it's empty we then add a random hash to the location bar.
